    ...
    <div class="wishlist-container">
        ...
        <div v-else class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-3 lg:grid-cols-4 gap-4 md:gap-6">
            ...
            <div class="px-4 pb-4 mt-auto">
                ...
                <div v-if="!isInStock(getProduct(item))">
                    <button disabled
                        class="w-full bg-gray-200 text-gray-400 font-medium py-2.5 rounded-xl cursor-not-allowed text-sm">
                        {{ t('unavailable') }}
                    </button>
                </div>
                ...
                <!-- <button
                    v-else-if="!isInCart(getProduct(item))"
                    @click="addToCart(getProduct(item))"
                    :disabled="isAddingToCart(getProduct(item).id)"
                    class="w-full bg-linear-to-r from-green-600 to-green-500 hover:from-green-700 hover:to-green-600 text-white font-semibold py-2.5 rounded-xl transition-all duration-300 flex items-center justify-center gap-2 shadow-md hover:shadow-lg disabled:opacity-60 disabled:cursor-not-allowed text-sm">
                ...
                <button
                    ...
                    <svg v-if="isAddingToCart(getProduct(item).id)"
                    ...
                    <span class="text-white">{{ t('add_to_cart') }}</span>
                </button>
                <!-- Already in cart → Quantity controls -->
                <div v-else class="quantity-control flex items-center border-2 border-green-600 rounded-xl overflow-hidden bg-white">
                    <button
                        @click="decrementQuantity(getProduct(item))"
                        :disabled="isUpdatingQuantity(getProduct(item).id) || !canDecrement(getProduct(item))"
                        class="px-3 py-2 text-lg font-bold text-green-700 hover:bg-green-50 transition-colors disabled:opacity-50 disabled:cursor-not-allowed">
                        −
                    </button>

                    <div class="flex-1 text-center py-2 bg-green-50/30 font-semibold relative min-w-[3rem] text-sm">
                        <span v-if="isUpdatingQuantity(getProduct(item).id)"
                            class="absolute inset-0 flex items-center justify-center bg-white bg-opacity-75">
                            <div class="animate-spin rounded-full h-4 w-4 border-2 border-green-500 border-t-transparent"></div>
                        </span>
                        <span :class="{ 'opacity-0': isUpdatingQuantity(getProduct(item).id) }">
                            {{ formatQuantity(getProduct(item)) }}
                        </span>
                    </div>

                    <button
                        @click="incrementQuantity(getProduct(item))"
                        :disabled="isUpdatingQuantity(getProduct(item).id) || !canIncrement(getProduct(item))"
                        class="px-3 py-2 text-lg font-bold text-green-700 hover:bg-green-50 transition-colors disabled:opacity-50 disabled:cursor-not-allowed">
                        +
                    </button>
                </div>
...
// ========== Quantity helpers (same logic as seeds component) ==========
...
const getUnitConfig = (unit) => {
    const configs = {
        ...
        kg:    { step: 0.5, min: 0.5 },
        g:     { step: 100, min: 100 },
        liter: { step: 0.5, min: 0.5 },
    };
    return configs[unit] || configs.piece;
};

const convertToGrams = (product, quantity) => {
    const unit = product.unit || 'piece';
    if (['piece', 'pack', 'bunch'].includes(unit)) return Math.round(quantity);
    if (unit === 'kg' || unit === 'liter') return Math.round(quantity * 1000);
...
const getCurrentQuantityInUnit = (product) => {
    ...
    const unit = product.unit || 'piece';
    if (['piece', 'pack', 'bunch'].includes(unit)) {
...
const canIncrement = (product) => {
    const current = getCurrentQuantityInUnit(product);
    const max = product.stock || 50;
    const step = getUnitConfig(product.unit).step;
    return current + step <= max;
};

const canDecrement = (product) => {
    const current = getCurrentQuantityInUnit(product);
    const min = getUnitConfig(product.unit).min;
    const step = getUnitConfig(product.unit).step;
...
const isUpdatingQuantity = (productId) => {
    ...
    return localUpdatingItems.value.has(item.id);
};
...
const updateQuantity = async (itemId, productId, quantityGrams) => {
    ...
    try {
        await store.dispatch('updateQuantity', {
            itemId,
            quantityGrams: Math.round(quantityGrams)
        });
    } catch (error) {
...
const incrementQuantity = async (product) => {
    ...
    const newGrams = convertToGrams(product, newVal);
    await updateQuantity(cartItem.id, product.id, newGrams);
};
...
const addToCart = async (product) => {
    ...
    try {
        ...
        const quantityInGrams = convertToGrams(product, config.min);
