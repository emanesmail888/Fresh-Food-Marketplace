                                    ...
                                    <tr v-for="item in cartItems" :key="item.id"
                                        ...
                                        <td class="py-4">
                                            <div class="flex items-start gap-4">
                                                <router-link :to="getProductLink(item.product)" class="shrink-0">
                                                ...
                                                <div>
                                                    ...
                                                    <p v-if="item.product?.stock" class="text-xs mt-1"
                                                        :class="isLowStock(item.product) ? 'text-orange-500' : 'text-green-600'">
                                                        {{ isLowStock(item.product) ? t("low_stock") : t("in_stock") }}
                                                    </p>
                                                </div>
                                            </div>
                                        </td>
                                        ...
                                        <td class="py-4 align-middle">
                                            <div
                                                ...
                                                <button @click="decrementQuantity(item)"
                                                    :disabled="isUpdatingQuantity(item.id) || !canDecrement(item)"
                                                ...
                                                <div
                                                    class="flex-1 text-center py-2 bg-green-50/30 font-semibold relative">
                                                    ...
                                                    <span :class="{ 'opacity-0': isUpdatingQuantity(item.id) }">
                                                        {{ getQuantityDisplay(item) }}
                                                    </span>
                                                </div>
                                                <button @click="incrementQuantity(item)"
                                                    :disabled="isUpdatingQuantity(item.id) || !canIncrement(item)"
                                                    class="px-3 py-2 text-xl font-bold text-green-700 hover:bg-green-50 transition-colors disabled:opacity-50 disabled:cursor-not-allowed">
                                                    +
                                                </button>
                                            </div>
                                        ...
                                        <td class="py-4 align-middle">
                                            <button @click="removeItem(item.id)" :disabled="isRemovingItem(item.id)"
                                                class="inline-flex items-center justify-center w-8 h-8 bg-white rounded-full text-gray-400 hover:text-green-600 hover:bg-green-50 transition-colors disabled:opacity-50">
                        ...
                        <div class="md:hidden space-y-4">
                            <div v-for="item in cartItems" :key="item.id"
                                ...
                                <div class="flex gap-3">
                                    <!-- Image -->
                                    <router-link :to="getProductLink(item.product)" class="shrink-0">
                                        <img v-if="item.product?.image" :src="`${storageBase}/${item.product.image}`"
                                            :alt="item.product.title" class="w-20 h-20 object-cover rounded-xl"
                                            loading="lazy" />
                                        <div v-else
                                            class="w-20 h-20 bg-gray-100 rounded-xl flex items-center justify-center text-gray-400 text-xs">
                                            {{ t("no_image") }}
                                        </div>
                                    </router-link>
                                    ...
                                    <div class="flex-1 min-w-0">
                                        <div class="flex justify-between items-start gap-2">
                                            <router-link :to="getProductLink(item.product)"
                                                class="text-sm font-semibold text-gray-800 hover:text-green-600 line-clamp-2">
                                                {{ currentLanguage === 'ar' && item.product?.title_ar
                                                    ? item.product.title_ar
                                                    : item.product?.title || t("Product") }}
                                            </router-link>
                                            ...
                                            <!-- Remove button -->
                                            <button @click="removeItem(item.id)" :disabled="isRemovingItem(item.id)"
                                                class="shrink-0 w-8 h-8 flex items-center justify-center rounded-full text-gray-400 hover:text-red-500 hover:bg-red-50 transition-colors">
                                                <span v-if="isRemovingItem(item.id)">⌛</span>
                                ...
                                <div class="flex items-center justify-between mt-4 pt-3 border-t border-gray-100">
                                    ...
                                    <div
                                        ...
                                        <button @click="decrementQuantity(item)"
                                            :disabled="isUpdatingQuantity(item.id) || !canDecrement(item)"
                                            class="px-3 py-2 text-lg font-bold text-green-700 hover:bg-green-50 disabled:opacity-50">
                                            −
                                        </button>
                                        <div
                                            class="w-14 text-center py-2 bg-green-50/40 font-semibold text-sm relative">
                                            <span v-if="isUpdatingQuantity(item.id)"
                                    ...
                                    <div class="text-right">
                                        <p class="text-xs text-gray-400">{{ t("total") }}</p>
